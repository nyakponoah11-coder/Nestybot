 ...
 Choose an option to continue"
      ...
      if (
        ...
        await supabase
          .from("sessions")
          .update({
 Enter a correct Ghana phone number to continue"
            phone_number: phone,
            step: 8
          })
          .eq(
...
(This can be the same number or a different one)`
 ...
 Enter a correct Ghana Mobile Money number to continue"
        const {
 ...
 Cancelled\n\n" +
      ...
      if (
        ...
        await supabase
          .from("sessions")
          .update({
 Enter an amount between ₵${MASHUP_MIN_AMOUNT} and ₵${MASHUP_MAX_AMOUNT} (e.g. 5)`
            bundle: amountText,
            step: 11
          })
          .eq(
...
Reply YES to pay or NO to cancel`
 ...
 Reply 1 for MTN, 2 for AirtelTigo, or 3 for Telecel"
        return sendWhatsApp(
          from,
      ...
      if (
        ...
        await supabase
 Enter the applicant's full name (e.g. John Doe)"
          .from("sessions")
          .update({
            bundle:
              mergeAfaField(
      ...
      if (
        ...
        await supabase
 Enter a correct Ghana phone number (e.g. 0241234567)"
          .from("sessions")
          .update({
            bundle:
              mergeAfaField(
 ...
 Enter the applicant's location (e.g. Tafo, Kumasi)"
      ...
      if (
        ...
        if (!dob) {
          return sendWhatsApp(
            from,
 Use the format DD/MM/YYYY (e.g. 15/06/1995)"
            "Invalid date 
          );
        }
        await supabase
