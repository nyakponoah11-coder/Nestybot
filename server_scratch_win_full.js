...
const ADMIN_SCRATCH_SECRET = process.env.ADMIN_SCRATCH_SECRET;
...
async function generateUniqueScratchCode() {
  ...
  throw new Error("Could not generate a unique Scratch Code.");
}

async function getScratchCode(code) {
  const normalized = normalizeScratchCode(code);

  if (!/^DATA1[A-Z0-9]{6}$/.test(normalized)) {
    return null;
  }
  ...
  if (error) {
    console.error("SCRATCH CODE LOOKUP ERROR:", error);
    throw error;
  }
...
async function validateAndAttachScratchCode(from, code) {
  const normalized = normalizeScratchCode(code);
  const scratch = await getScratchCode(normalized);

  if (!scratch) {
    return {
      ok: false,
      message:
        "❌ Invalid Data1 Scratch Code.\n\n" +
        "Please check the code and enter it again."
    };
  }

  if (scratch.status === "used" || scratch.scratched) {
    return {
      ok: false,
      message:
        "❌ This Scratch Code has already been used."
    };
  }

  const customer = normalizePhone(from);
  const assigned = scratch.assigned_to
    ? normalizePhone(scratch.assigned_to)
    : null;

  if (assigned && assigned !== customer) {
    return {
      ok: false,
      message:
        "❌ This Scratch Code is already assigned to another customer."
    };
  }
  ...
  if (error) {
    console.error("SCRATCH CODE ATTACH ERROR:", error);

    return {
      ok: false,
      message:
        "❌ We could not attach your Scratch Code right now. Please try again."
    };
  }
  ...
  return {
    ok: true,
    code: normalized,
    ordersCompleted: Number(scratch.orders_completed || 0),
    unlocked: Boolean(scratch.unlocked)
  };
}

async function getActiveScratchCodeForCustomer(from) {
  const customer = normalizePhone(from);
  ...
  if (error) {
    console.error("ACTIVE SCRATCH LOOKUP ERROR:", error);
    return null;
  }
...
function scratchStatusMessage(scratch) {
  if (!scratch) {
    return (
      "🎟️ SCRATCH & WIN\n\n" +
      "You do not currently have an active Scratch Code.\n\n" +
      "If you have a Data1 Scratch Code, use it during payment when placing a data order."
    );
  }

  const completed = Number(scratch.orders_completed || 0);

  if (scratch.unlocked) {
    return (
      "🎉 SCRATCH CARD UNLOCKED!\n\n" +
      `🎟️ Code: ${scratch.code}\n` +
      `💳 Paid orders: ${completed}/${SCRATCH_REQUIRED_ORDERS} ✅\n\n` +
      "Reply SCRATCH to play and win 1GB or 2GB DATA! 🎁"
    );
  }
  ...
  return (
    ...
    `🎟️ Code: ${scratch.code}\n` +
    `💳 Paid orders: ${completed}/${SCRATCH_REQUIRED_ORDERS}\n\n` +
    `You need ${Math.max(0, SCRATCH_REQUIRED_ORDERS - completed)} more qualifying paid order(s) to unlock your Scratch Card.`
  );
}

async function countScratchPaidOrder(from, scratchCode, orderReference) {
  if (!scratchCode || !orderReference) return null;
  ...
  if (!scratch) {
    console.error("SCRATCH CODE NOT FOUND WHILE COUNTING:", normalized);
    return null;
  }

  if (scratch.scratched || scratch.status === "used") {
    return null;
  }

  const completed = Math.min(
    SCRATCH_REQUIRED_ORDERS,
    Number(scratch.orders_completed || 0) + 1
  );

  const unlocked = completed >= SCRATCH_REQUIRED_ORDERS;

  const { error } = await supabase
    .from("scratch_codes")
    .update({
      assigned_to: normalizePhone(from),
      orders_completed: completed,
      unlocked,
      status: unlocked ? "unlocked" : "active",
      unlocked_at:
        unlocked && !scratch.unlocked
          ? new Date().toISOString()
          : scratch.unlocked_at
    })
    .eq("id", scratch.id);
  ...
  if (error) {
    console.error("SCRATCH PROGRESS UPDATE ERROR:", error);
    return null;
  }

  console.log(
    `🎟️ SCRATCH PROGRESS ${normalized}: ${completed}/${SCRATCH_REQUIRED_ORDERS}`
  );
...
async function deliverScratchPrize(
  from,
  scratch,
  network,
  deliveryPhone
) {
  const packageInfo = PACKAGES[network]?.["1"];

  if (!packageInfo) {
    throw new Error("Scratch network configuration not found.");
  }

  const prize = scratch.prize;
  const capacity = prize === "2GB" ? "2" : "1";

  /*
   * Uses the same DataMart network value currently used
   * by your normal data packages.
   */
  const networkConfig = PACKAGES[network]?.["1"];

  const response = await axios.post(
    `${DATAMART_BASE}/purchase`,
    {
      phoneNumber: normalizePhone(deliveryPhone),
      network: networkConfig.apiNetwork,
      capacity,
      gateway: "wallet"
    },
    {
  ...
  await supabase
    ...
    .update({
      ...
      scratched: true,
      network,
      delivery_phone: normalizePhone(deliveryPhone),
      datamart_reference: reference,
      datamart_order_id: orderId,
...
const scratch =
await getActiveScratchCodeForCustomer(from);
...
Do you have a Data1 Scratch Code for this order?

1 - YES, use my code
2 - NO / SKIP

Your Scratch Code is optional.`
);
}

/* =====================================================
STEP 7 - OPTIONAL SCRATCH CODE
===================================================== */
...
🎟️ Scratch & Win: Not participating in this order.
...
`🎟️ ENTER YOUR DATA1 SCRATCH CODE
...
return sendWhatsApp(
from,
"Reply 1 for YES, use a Scratch Code, or 2 for NO / SKIP."
);
}

/* =====================================================
STEP 15 - VALIDATE SCRATCH CODE FOR THIS ORDER
===================================================== */
...
const result =
await validateAndAttachScratchCode(
from,
text
);
...
`✅ SCRATCH CODE ACCEPTED!

🎟️ Code: ${result.code}
💳 Current progress: ${result.ordersCompleted}/${SCRATCH_REQUIRED_ORDERS}

This paid order will count toward your Scratch & Win challenge if payment succeeds.
...
return sendWhatsApp(
from,
"❌ You do not have an active Scratch Card."
);
}
...
console.error(
"SCRATCH PRIZE SAVE ERROR:",
error
);

return sendWhatsApp(
from,
"❌ We could not open your Scratch Card. Please try again."
);
}
...
📶 Choose the network for your FREE ${prize.prize}:
...
`🎁 Your prize: ${scratch?.prize || "DATA"}

📶 Network: ${network}

Enter the Ghana phone number you want the FREE data sent to:
...
/* =====================================================
STEP 62 - SCRATCH DELIVERY NUMBER
===================================================== */
...
return sendWhatsApp(
from,
"❌ Your Scratch Card is not available. Reply HI to continue."
);
}
...
await sendWhatsApp(
from,
`🎁 Sending your FREE ${scratch.prize} data now...
...
Your Scratch Code has now been used.

Thank you for using Data1gh 🇬🇭
...
`❌ We could not deliver your ${scratch.prize} prize right now.

Your Scratch Card has NOT been marked as used.
...
/*
 * Only a successful regular Data order with an
 * optional Scratch Code counts toward the 5-order
 * Scratch & Win challenge.
 */
if (
session.scratch_code &&
datamartReference &&
["MTN", "AIRTELTIGO", "TELECEL"].includes(session.network)
) {

const scratchProgress =
await countScratchPaidOrder(
session.phone,
session.scratch_code,
datamartReference
);

if (scratchProgress?.unlocked) {
...
🎟️ Scratch Code: ${scratchProgress.code}

💳 Paid orders completed:
${SCRATCH_REQUIRED_ORDERS}/${SCRATCH_REQUIRED_ORDERS} ✅

🔓 YOUR SCRATCH CARD IS NOW UNLOCKED!

After your order is delivered, reply SCRATCH to play and win 1GB or 2GB DATA! 🎁`
);

} else if (scratchProgress) {

await sendWhatsApp(
session.phone,

`🎟️ SCRATCH & WIN PROGRESS

Code: ${scratchProgress.code}

💳 Paid orders:
${scratchProgress.completed}/${SCRATCH_REQUIRED_ORDERS}

You need ${SCRATCH_REQUIRED_ORDERS - scratchProgress.completed} more qualifying paid order(s) to unlock your Scratch Card.`
);
...
/*
 * Clear the session's temporary Scratch Code after
 * this paid order. The code remains assigned in
 * scratch_codes and the customer must explicitly
 * choose YES + enter the code again on the next order.
 */
await supabase
.from("sessions")
.update({
...
/* =========================================================
SCRATCH CODE ADMIN API
========================================================= */

/*
Generate unique customer Scratch Codes.

POST:
  /admin/scratch-codes/generate

Header:
  x-admin-secret: YOUR_ADMIN_SCRATCH_SECRET
...
const code =
await generateUniqueScratchCode();
